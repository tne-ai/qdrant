...
def test_partial_snapshot_recovery_lock(tmp_path: pathlib.Path):
    ...
    assert any(response.status_code == 500 for response in responses), "Subsequent partial snapshot recovery requests have to be rejected during partial snapshot recovery"
