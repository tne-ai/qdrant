...
/// - Initial index building. (the graph is stored in the cache)
/// - Exact search. (the results are stored in the cache)
#[derive(Parser, Debug)]
...
fn main() {
    ...
    let cache_index_path = cache_path.join(format!(
    ...
    let mut rng = StdRng::from_rng(&mut main_rng);
    let mut last_segment = make_segment(&mut rng, tmp_dir.path(), &vectors, sliding_window.clone());
    ...
    for iteration in 0..args.iterations {
        ...
        let segment = make_segment(&mut rng, tmp_dir.path(), &vectors, sliding_window.clone());
        let index = build_hnsw_index(
            &mut rng,
